...
export async function POST(req: Request) {
  ...
  // Squad-number uniqueness is scoped per product variant — two players can hold
  // #7 across different colour jerseys, but not within the same SKU.
  const intraOrderPlayers = new Map<string, { number: string; name: string }>();
  for (const item of data.items) {
    ...
    const key = `${item.productId}:${num}`;
    const existing = intraOrderPlayers.get(key);
    ...
    intraOrderPlayers.set(key, { number: num, name: nm });
  }

  const supabase = createAdminClient();

  // Cross-order conflict: another customer already claimed this number on the
  // same product variant.
  if (intraOrderPlayers.size > 0) {
    const numbers = Array.from(new Set(Array.from(intraOrderPlayers.values()).map((v) => v.number)));
    const productIds = Array.from(
      new Set(Array.from(intraOrderPlayers.keys()).map((k) => k.split(':')[0])),
    );
    ...
    for (const raw of (existingClaims ?? []) as unknown as Row[]) {
      ...
      const k = `${raw.product_id}:${raw.player_number.trim()}`;
      if (!seen.has(k)) seen.set(k, raw.player_name.trim().toUpperCase());
    }

    for (const [key, { number, name }] of intraOrderPlayers) {
      const claimed = seen.get(key);
      if (claimed && claimed !== name) {
        return NextResponse.json(
          { error: `Number ${number} is already taken by ${claimed}` },
          { status: 400 },
        );
      }
    }
